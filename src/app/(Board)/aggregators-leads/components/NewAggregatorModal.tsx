...
export function NewAggregatorModal({
  ...
  useEffect(() => {
    if (draft.cropName) {
      // Parse cropName - could be single value or comma-separated
      const crops =
        typeof draft.cropName === "string"
          ? draft.cropName
              .split(",")
              .map((c) => c.trim())
              .filter(Boolean)
    ...
    try {
      ...
      if (Array.isArray(list) && list.length > 0) {
        const exact = list.find(
          (x: any) => String(x.number) === String(d.number)
        );
        return exact || list[0];
      }
    } catch (err) {
    ...
    try {
      ...
      if (draft.id) {
        ...
        try {
          ...
          console.error(
            "PATCH error response body:",
            err?.response?.data ?? err?.data ?? err?.message
          );
          throw err;
        }
        ...
        try {
          if (
            ...
            await updateBuyerType(cleanedDraft.userId as any, {
      ...
      if (found && (found.id || found._id || found.leadId)) {
        ...
        try {
          ...
          console.error(
            "PATCH(found) error response body:",
            err?.response?.data ?? err?.data ?? err?.message
          );
          throw err;
        }
      ...
      if (saveDraft) {
        try {
          ...
          console.error(
            "create via parent saveDraft error:",
            err?.response?.data ?? err?.data ?? err?.message
          );
          ...
          if (
            ...
            serverMsg.includes("Aggregator lead already exists")
          ) {
            ...
            if (found2 && (found2.id || found2._id || found2.leadId)) {
              ...
              const { cleanedDraft, payload } =
                buildCleanedDraftAndPayload(draft);
              const sanitized = sanitizePayload(payload ?? {});
              try {
                await patchAggregatorLead(String(foundId), sanitized);
                if (onSaved) await onSaved();
                ...
                console.error(
                  "Recovery patch failed:",
                  err2?.response?.data ?? err2?.data ?? err2?.message
                );
                throw err2;
              }
            }
      ...
      alert(
        "Failed to save aggregator. Check console/network tab for server response."
      );
    } finally {
    ...
    >
      <div className="bg-white rounded-lg shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div>
                <div className="flex justify-between items-baseline">
                  <label className="text-xs text-gray-500">Crop(s) *</label>
                          ...
                          >
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                // Update draft.cropName with comma-separated values
                                updateDraftField(
                                  "cropName",
                                  newSelected.join(", ")
                                );
        ...
        <div className="sticky bottom-0 bg-gray-50 border-t border-gray-200 p-4 flex justify-end gap-3">
          ...
          <button
            onClick={handleSave}
            disabled={savingLocal || savingLead}
