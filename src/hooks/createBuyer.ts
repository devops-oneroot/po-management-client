...
const genUserBuyer = async (data: any): Promise<{ user: any; token: string } | null> => {
  try {
    ...
    console.error("❌ Error in buyer signup:", error.response?.data || error.message);
    alert("Error while creating buyer account");
    throw error;
  }
};

const addPrice = async (priceData: any, token: string, userId: string): Promise<void> => {
  ...
  try {
    ...
    for (const priceData of data.prices) {
      ...
      if (!buyerInfo.cropNames.includes(priceData.cropName)) {
        console.warn(`⚠️ Skipping price for ${priceData.cropName}: not in cropNames`);
        continue;
      }
    ...
    return { status: 200, user, message: "Buyer and prices created successfully" };
  } catch (error: any) {
    console.error("💥 Buyer creation process failed:", error.response?.data || error.message);
